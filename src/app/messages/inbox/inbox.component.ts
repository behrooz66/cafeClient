...
export class InboxComponent implements OnInit {
  ...
  {
      this.waiting = true;
      this._msg.getMessagesByReceiver(this._auth.getUserId())
          .subscribe(
              d => {
                  this.messages = d;
                  this.pageSetup();
  ...
  markAsRead(id: number) 
  {
      this._msg.markAsRead(id)
        .subscribe(
            ...
            d => {
                this._flash.addMessage("Error", "Operatin failed.", true, "danger", 2500, 2);
            }
        );
  }

  markAsUnread(id: number) {
      this._msg.markAsUnread(id)
        .subscribe(
            d => {
                this.refresh();
                this._msg.next();
